...
def _soundex(text):
    ...
    Soundex encodes words by their sound, so "milk" and "milc" match.
    Returns 4-character code like 'M420'.
    """
    if not text:
        return ""
    ...
    for char in text[1:]:
        for key, value in soundex_map.items():
            if char in key:
                if value != result[-1]:  # Don't add consecutive duplicates
                    result += value
                break
...
def _levenshtein_distance(s1, s2):
    ...
    Returns the minimum number of single-character edits needed to transform s1 to s2.
    """
    if len(s1) < len(s2):
...
def _get_synonyms(term):
    """Get synonyms for a search term from Synonyms DocType."""
    try:
        synonyms = frappe.db.get_all(
            "SM Search Synonym",
            filters={"term": term},
            fields=["synonym"],
        )
        return [s.synonym for s in synonyms]
    except Exception:
        return []


def _expand_search_query(query):
    """Expand search query with synonyms and fuzzy variants.

    Returns a list of terms to search for, including original, synonyms,
    and fuzzy variations.
    """
    terms = [query]
    term_lower = query.lower()

    # Add synonyms if any exist
    synonyms = _get_synonyms(term_lower)
    terms.extend(synonyms)

    # Add Soundex match if search term is 4+ characters
    if len(query) >= 4:
        soundex_code = _soundex(query)
        # Find products with similar Soundex codes (could be expanded to full table scan)
        # For now, just add to terms for LIKE matching
        terms.append(f"%{query[:2]}%")  # Partial match

    return list(set(terms))


def _get_fuzzy_matches(term, field, table, limit=5):
    """Find fuzzy matches for a term using Levenshtein distance.

    Returns products with similar names based on edit distance.
    """
    # This would ideally use a MySQL UDF for Levenshtein
    # For now, use Soundex as a simpler alternative
    try:
        soundex = _soundex(term)
        results = frappe.db.sql(f"""
            SELECT DISTINCT {field}, {table}.name
            FROM `{table}`
            WHERE {field} IS NOT NULL
            AND SOUNDEX({field}) = %(soundex)s
            LIMIT %(limit)s
        """, {"soundex": soundex, "limit": limit}, as_dict=True)
        return results
    except Exception:
        return []
