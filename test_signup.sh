...
echo "=== 1. Signup ==="
curl -s -X POST "$BASE/method/saathimart.api.auth_full.signup" \
...
curl -s -X POST "$BASE/method/saathimart.api.auth_full.verify_signup_otp" \
  -H "Host: saathi-mw.localhost" \
  -H "Content-Type: application/json" \
...
echo ""
echo "=== 4. Login ==="
curl -s -X POST "$BASE/method/saathimart.api.auth_full.login" \
  -H "Host: saathi-mw.localhost" \
  -H "Content-Type: application/json" \
